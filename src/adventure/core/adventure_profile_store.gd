...
@private
static func _migrate_profile(profile: LumenfallSaveData) -> void:
	if profile.completed_quest_ids == null:
		profile.completed_quest_ids = [] as Array[String]
	if profile.inventory_item_ids == null:
		profile.inventory_item_ids = [] as Array[String]
	if profile.inventory_item_counts == null:
		profile.inventory_item_counts = [] as Array[int]
	if profile.unlocked_recipe_ids == null:
		profile.unlocked_recipe_ids = [] as Array[String]
	profile.schema_version = 1
