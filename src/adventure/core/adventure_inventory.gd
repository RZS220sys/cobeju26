...
static func count(profile: LumenfallSaveData, item_id: String) -> int:
...
static func add(profile: LumenfallSaveData, item_id: String, amount: int = 1) -> void:
	if amount <= 0:
		return
	for index: int in range(profile.inventory_item_ids.size()):
...
static func remove(profile: LumenfallSaveData, item_id: String, amount: int = 1) -> bool:
...
static func unlock_recipe(profile: LumenfallSaveData, recipe_id: String) -> void:
	if recipe_id not in profile.unlocked_recipe_ids:
		profile.unlocked_recipe_ids.append(recipe_id)


static func has_recipe(profile: LumenfallSaveData, recipe_id: String) -> bool:
