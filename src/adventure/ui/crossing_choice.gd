...
func _ready() -> void:
	...
	_add_choice(choices, &"shelter", "SHELTER", "Close the road for now. Hearthmere is safer, but the forgotten remain unreachable.", Color("70d0ef"))
	_add_choice(choices, &"bridge", "BRIDGE", "Bind the road to three living names. Travel stays possible, and those people carry its cost.", Color("e7b75c"))
	_add_choice(choices, &"witness", "WITNESS", "Cross without owning or closing it. Learn who erased the road before deciding its fate.", Color("c884e4"))
...
@private
func _add_choice(parent: HBoxContainer, choice_id: StringName, heading: String, copy: String, color: Color) -> void:
