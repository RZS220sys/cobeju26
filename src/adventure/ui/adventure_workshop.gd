...
@private
func _refresh() -> void:
	_showing_compass = AdventureInventory.has_recipe(profile, "memory_compass") and AdventureInventory.count(profile, "memory_compass") == 0
	if _showing_compass:
		_refresh_compass()
		return
	...
	if AdventureInventory.count(profile, "lantern_lens") > 0:
...
func _refresh_compass() -> void:
	var threads := AdventureInventory.count(profile, "memory_thread")
...
func craft_lantern_lens() -> void:
	if not AdventureInventory.remove(profile, "riftglass", 3):
		_refresh()
		return
	AdventureInventory.add(profile, "lantern_lens")
...
func craft_memory_compass() -> void:
	if not AdventureInventory.remove(profile, "memory_thread", 3):
		_refresh()
		return
	AdventureInventory.add(profile, "memory_compass")
