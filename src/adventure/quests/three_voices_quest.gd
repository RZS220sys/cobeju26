...
const VOICE_ITEMS: Dictionary[StringName, String] = {
	&"iven_voice": "causeway_iven_voice",
	&"sola_voice": "causeway_sola_voice",
	&"orin_voice": "causeway_orin_voice",
}
...
func handle_interaction(interactable: Node3D) -> void:
	if interactable is AdventureWorldInteractable:
		...
		if profile.quest_stage == 18 and world_object.interaction_id == &"waystone":
			_begin_causeway()
		elif profile.quest_stage == 19 and world_object.interaction_id in VOICE_ITEMS:
			_awaken_voice(world_object.interaction_id)
		elif profile.quest_stage == 20 and world_object.interaction_id == &"three_voices_gate":
			_open_crossing_choice()
	elif interactable is AdventureNpc:
		var npc := interactable as AdventureNpc
		if profile.quest_stage == 21 and npc.npc_id == &"nia":
			_complete_crossing()


@private
func _begin_causeway() -> void:
	profile.quest_stage = 19
	profile.active_quest_id = "crossing_of_three_voices"
	world.enter_echo_causeway()
	_restore_altars()
	_update_causeway_objective()
...
@private
func _awaken_voice(altar_id: StringName) -> void:
	var item_id: String = VOICE_ITEMS[altar_id]
	...
	if _voice_count() >= 3:
		profile.quest_stage = 20
		profile.active_quest_id = "the_road_asks"
...
@private
func _on_crossing_choice(choice_id: StringName) -> void:
	AdventureInventory.add(profile, "crossing_choice_%s" % choice_id)
	profile.quest_stage = 21
	profile.active_quest_id = "answer_carried_home"
...
func _complete_crossing() -> void:
	profile.quest_stage = 22
	profile.active_quest_id = "hearthmere_after_the_answer"
	profile.completed_quest_ids.append("crossing_of_three_voices")
...
func _restore_altars() -> void:
	if not is_instance_valid(world.echo_causeway):
		return
	for altar_id: StringName in VOICE_ITEMS:
...
func _voice_count() -> int:
	var count := 0
	for item_id: String in VOICE_ITEMS.values():
		if AdventureInventory.count(profile, item_id) > 0:
			count += 1
	return count
