class_name AdventureProfileScreen
extends Control
...
signal profile_selected(profile: LumenfallSaveData)
...
var _pending_action: ProfileAction = ProfileAction.NONE
...
func _build_interface() -> void:
	...
	_confirm_dialog.confirmed.connect(_confirm_profile_action)
...
func _build_creation_panel() -> PanelContainer:
	...
	_name_edit.text_submitted.connect(func(_text: String) -> void: _create_profile())
...
func _refresh_profiles() -> void:
	for child: Node in _profile_list.get_children():
		child.queue_free()
	var index := AdventureProfileStore.load_index()
	_empty_hint.visible = index.profiles.is_empty()
	for summary: LumenfallProfileSummary in index.profiles:
		if is_instance_valid(summary):
			_add_profile_card(summary)
	if index.profiles.is_empty():
		_show_creation.call_deferred()


@private
func _add_profile_card(summary: LumenfallProfileSummary) -> void:
	var row := HBoxContainer.new()
	row.add_theme_constant_override(&"separation", 8)
	_profile_list.add_child(row)
	var load_button := Button.new()
	load_button.name = "Load_%s" % summary.profile_id
	...
	load_button.pressed.connect(_load_profile.bind(summary.profile_id))
	row.add_child(load_button)
	var reset_button := Button.new()
	reset_button.text = "↺"
	reset_button.tooltip_text = "Start this traveler over"
	reset_button.custom_minimum_size.x = 55.0
	reset_button.pressed.connect(_reset_profile.bind(summary.profile_id))
	row.add_child(reset_button)
	var delete_button := Button.new()
	delete_button.text = "✕"
	delete_button.tooltip_text = "Delete this traveler"
	delete_button.custom_minimum_size.x = 55.0
	delete_button.pressed.connect(_delete_profile.bind(summary.profile_id))
...
func _create_profile() -> void:
	var profile := AdventureProfileStore.create_profile(_name_edit.text)
	profile_selected.emit(profile)


@private
func _load_profile(profile_id: String) -> void:
	var profile := AdventureProfileStore.load_profile(profile_id)
	if is_instance_valid(profile):
		profile_selected.emit(profile)


@private
func _reset_profile(profile_id: String) -> void:
	_pending_profile_id = profile_id
	_pending_action = ProfileAction.RESET
...
func _delete_profile(profile_id: String) -> void:
	_pending_profile_id = profile_id
	_pending_action = ProfileAction.DELETE
...
@private
func _confirm_profile_action() -> void:
	if _pending_profile_id.is_empty():
		return
	if _pending_action == ProfileAction.RESET:
		AdventureProfileStore.reset_profile(_pending_profile_id)
	elif _pending_action == ProfileAction.DELETE:
		AdventureProfileStore.delete_profile(_pending_profile_id)
	_pending_profile_id = ""
	_pending_action = ProfileAction.NONE
	_refresh_profiles()
