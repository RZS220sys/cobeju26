...
signal choice_made(choice_id: LumenfallTypes.CrossingChoice)
...
func _ready() -> void:
	...
	_add_choice(choices, LumenfallTypes.CrossingChoice.SHELTER, "SHELTER", "Close the road for now. Hearthmere is safer, but the forgotten remain unreachable.", Color("70d0ef"))
	_add_choice(choices, LumenfallTypes.CrossingChoice.BRIDGE, "BRIDGE", "Bind the road to three living names. Travel stays possible, and those people carry its cost.", Color("e7b75c"))
	_add_choice(choices, LumenfallTypes.CrossingChoice.WITNESS, "WITNESS", "Cross without owning or closing it. Learn who erased the road before deciding its fate.", Color("c884e4"))
...
@private
func _add_choice(parent: HBoxContainer, choice_id: LumenfallTypes.CrossingChoice, heading: String, copy: String, color: Color) -> void:
...
func make_choice(choice_id: LumenfallTypes.CrossingChoice) -> void:
