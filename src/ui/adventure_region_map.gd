...
signal region_selected(region_id: LumenfallTypes.RegionId)
signal closed
...
func _build_interface() -> void:
	...
	_add_region_card(regions, LumenfallTypes.RegionId.GLASSWOOD, "NORTH  •  GLASSWOOD", "A crystal shrine is repeating Aster's name in voices from several decades.", LumenfallTypes.ItemId.GLASSWOOD_RESONANCE, Color("72ddff"))
	_add_region_card(regions, LumenfallTypes.RegionId.AMBERFEN, "EAST  •  AMBERFEN", "An abandoned sky-orrery has begun tracking something beneath the horizon.", LumenfallTypes.ItemId.AMBER_ORBIT, Color("f0b35b"))
	_add_region_card(regions, LumenfallTypes.RegionId.BELLSCAR, "SOUTH  •  BELLSCAR", "The great warning bell casts a moving shadow, though the bell itself is still.", LumenfallTypes.ItemId.BELLSCAR_ECHO, Color("e17d78"))
...
@private
func _add_region_card(parent: HBoxContainer, region_id: LumenfallTypes.RegionId, heading: String, description: String, discovery_item: LumenfallTypes.ItemId, color: Color) -> void:
...
func choose_region(region_id: LumenfallTypes.RegionId) -> void:
