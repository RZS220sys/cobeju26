...
@private
func _refresh() -> void:
	_showing_compass = AdventureInventory.has_recipe(profile, LumenfallTypes.RecipeId.MEMORY_COMPASS) and AdventureInventory.count(profile, LumenfallTypes.ItemId.MEMORY_COMPASS) == 0
	if _showing_compass:
		_refresh_compass()
		return
	...
	if AdventureInventory.count(profile, LumenfallTypes.ItemId.LANTERN_LENS) > 0:
...
func _refresh_compass() -> void:
	var threads := AdventureInventory.count(profile, LumenfallTypes.ItemId.MEMORY_THREAD)
...
func craft_lantern_lens() -> void:
	if not AdventureInventory.remove(profile, LumenfallTypes.ItemId.RIFTGLASS, 3):
		_refresh()
		return
	AdventureInventory.add(profile, LumenfallTypes.ItemId.LANTERN_LENS)
...
func craft_memory_compass() -> void:
	if not AdventureInventory.remove(profile, LumenfallTypes.ItemId.MEMORY_THREAD, 3):
		_refresh()
		return
	AdventureInventory.add(profile, LumenfallTypes.ItemId.MEMORY_COMPASS)
