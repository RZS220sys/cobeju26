...
func _ready() -> void:
	...
	collision_layer = LumenfallTypes.PhysicsLayer.ENEMY
	...
	add_child(collision)
	_model = AdventureAssetLibrary.instantiate_model(LumenfallTypes.AssetId.RIFT_HOUND)
...
func _spawn_impact(impact_color: Color) -> void:
	...
	if not is_instance_valid(parent):
		return
	var burst := AdventureImpactBurst.new()
