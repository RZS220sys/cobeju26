class_name RiftHoundNpc
extends BeastNpc

signal defeated(hound: RiftHoundNpc)
...
func identity_id() -> NpcCatalog.Id:
	return NpcCatalog.Id.RIFT_HOUND
...
func identity_model() -> ModelCatalog.Id:
	return ModelCatalog.Id.RIFT_HOUND
...
func _ready() -> void:
	...
	collision_layer = PhysicsLayers.Id.ENEMY
...
func build_visual() -> void:
	...
	add_child(_model)
	QuadrupedRigBinder.bind(_model)
...
func configure_behavior() -> void:
	...
	persistent_state.mood = NpcCatalog.Mood.AFRAID
...
func _physics_process(delta: float) -> void:
	...
	if _windup > 0.0:
		...
		_chase(to_target.normalized(), delta)
...
func on_defeated() -> void:
	...
	persistent_state.behavior = NpcCatalog.Behavior.RECOVERING
...
func _build_telegraph() -> void:
	...
	var material := ModelLibrary.material(Color("c92848"), 0.4, Color("e62f53"), 1.8)
...
func _return_home(delta: float) -> void:
	...
	if offset.length() > 0.8:
		_chase(offset.normalized() * (4.2 / 3.15), delta)
	else:
		velocity.x = 0.0
		velocity.z = 0.0
	move_and_slide()
...
@private
func _spawn_impact(impact_color: Color) -> void:
	var parent := get_parent() as Node3D
	if not is_instance_valid(parent):
		return
	var burst := ImpactBurst.new()
	burst.configure(impact_color)
	parent.add_child(burst)
	burst.global_position = global_position + Vector3.UP
