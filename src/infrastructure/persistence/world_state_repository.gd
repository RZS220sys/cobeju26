...
static func load_state(world_id: String) -> GameWorldState:
	...
	if not is_instance_valid(file):
		return null
	var decoded := GameWorldState.deserialize_binary(file.get_buffer(file.get_length()))
	file.close()
...
static func save_state(state: GameWorldState) -> bool:
