...
func _build_interface() -> void:
	...
	_add_region_card(regions, RegionCatalog.Id.GLASSWOOD, "NORTH  •  GLASSWOOD", "A crystal shrine is repeating Aster's name in voices from several decades.", ItemCatalog.Id.GLASSWOOD_RESONANCE, Color("72ddff"))
	_add_region_card(regions, RegionCatalog.Id.AMBERFEN, "EAST  •  AMBERFEN", "An abandoned sky-orrery has begun tracking something beneath the horizon.", ItemCatalog.Id.AMBER_ORBIT, Color("f0b35b"))
	_add_region_card(regions, RegionCatalog.Id.BELLSCAR, "SOUTH  •  BELLSCAR", "The great warning bell casts a moving shadow, though the bell itself is still.", ItemCatalog.Id.BELLSCAR_ECHO, Color("e17d78"))
...
func _add_region_card(parent: HBoxContainer, region_id: RegionCatalog.Id, heading: String, description: String, discovery_item: ItemCatalog.Id, color: Color) -> void:
	...
	var completed := Inventory.count(world_state, discovery_item) > 0
...
func choose_region(region_id: RegionCatalog.Id) -> void:
