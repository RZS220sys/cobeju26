...
func set_adventure_camera(camera: Camera3D) -> void:
	_camera = camera
...
func _ready() -> void:
	name = "Wayfarer"
	collision_layer = LumenfallTypes.PhysicsLayer.PLAYER
	collision_mask = LumenfallTypes.PhysicsLayer.WORLD
...
func _build_player() -> void:
	...
	add_child(collision)
	_avatar = AdventureAssetLibrary.instantiate_model(LumenfallTypes.AssetId.WAYFARER)
	if is_instance_valid(_avatar):
		_avatar.name = "WayfarerAvatar"
		_avatar.rotation.y = PI
		add_child(_avatar)
		AdventureRigBinder.bind_humanoid(_avatar)
...
func _build_drawn_sword() -> void:
	...
	grip.mesh = grip_mesh
	grip.material_override = AdventureAssetLibrary.material(Color("4a2818"), 0.8)
	...
	guard.material_override = AdventureAssetLibrary.material(Color("c48b34"), 0.3, Color("e2ad52"), 0.45)
	...
	blade.material_override = AdventureAssetLibrary.material(Color("b9dce4"), 0.2, Color("62cce8"), 0.75)
...
func _physics_process(delta: float) -> void:
	...
	elif input_enabled and Input.is_action_just_pressed(&"adventure_jump"):
	...
	var target_speed := sprint_speed if Input.is_action_pressed(&"adventure_sprint") else walk_speed
...
func take_damage(amount: float, source_position: Vector3) -> void:
	...
	var impact := AdventureImpactBurst.new()
