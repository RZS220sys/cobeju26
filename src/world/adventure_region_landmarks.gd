class_name AdventureRegionLandmarks
extends Node3D

var streamer: AdventureWorldStreamer
var landmarks: Dictionary[LumenfallTypes.InteractionId, AdventureWorldInteractable] = {}


func configure(streamer_value: AdventureWorldStreamer) -> void:
...
func _build_glasswood_shrine() -> void:
	...
	for index: int in range(7):
		...
		crystal.material_override = AdventureAssetLibrary.material(Color("447b92"), 0.24, Color("3aa8d2"), 1.4)
	...
	_add_interactable(LumenfallTypes.InteractionId.GLASSWOOD_SHRINE, "Listen at the Glasswood Shrine", center, "Glasswood Shrine")
...
func _build_amberfen_orrery() -> void:
	...
	for index: int in range(3):
		...
		ring.material_override = AdventureAssetLibrary.material(Color("b67829"), 0.32, Color("d18d34"), 0.7)
		root.add_child(ring)
	...
	core.mesh = core_mesh
	core.position.y = 5.7
	core.material_override = AdventureAssetLibrary.material(Color("f0aa3c"), 0.22, Color("e99628"), 2.0)
	root.add_child(core)
	_add_interactable(LumenfallTypes.InteractionId.AMBERFEN_ORRERY, "Turn the Amberfen Orrery", center, "Amberfen Orrery")
...
func _build_bellscar_bell() -> void:
	...
	bell.mesh = bell_mesh
	bell.position.y = 4.7
	bell.material_override = AdventureAssetLibrary.material(Color("6f4b37"), 0.38, Color("7d392f"), 0.32)
	root.add_child(bell)
	_add_cylinder(root, Vector3(0, 2.5, 0), 0.3, 4.2, Color("2b2928"), 10)
	_add_interactable(LumenfallTypes.InteractionId.BELLSCAR_BELL, "Touch the silent Bellscar bell", center, "Bellscar Wake")
...
func _build_glass_road_beacon() -> void:
	...
	for index: int in range(5):
		...
		shard.material_override = AdventureAssetLibrary.material(Color("d3edf1"), 0.16, Color("74d8e8"), 1.7)
	...
	_add_interactable(LumenfallTypes.InteractionId.GLASS_ROAD_BEACON, "Inspect the western glass-road beacon", center, "Glass Road Beacon")


@private
func _add_interactable(id_value: LumenfallTypes.InteractionId, prompt: String, at: Vector3, node_name: String) -> void:
...
func _add_box(parent: Node3D, at: Vector3, size: Vector3, color: Color, rotation_value: Vector3 = Vector3.ZERO) -> void:
	...
	visual.material_override = AdventureAssetLibrary.material(color, 0.9)
...
func _add_cylinder(parent: Node3D, at: Vector3, radius: float, height: float, color: Color, segments: int) -> void:
	...
	visual.mesh = mesh
	visual.position = at
	visual.material_override = AdventureAssetLibrary.material(color, 0.88)
	parent.add_child(visual)
