...
var altars: Dictionary[LumenfallTypes.InteractionId, AdventureWorldInteractable] = {}
...
func _build_causeway() -> void:
	...
	var altar_data: Dictionary[LumenfallTypes.InteractionId, Vector3] = {
		LumenfallTypes.InteractionId.IVEN_VOICE: Vector3(-24.0, 0.0, -18.0),
		LumenfallTypes.InteractionId.SOLA_VOICE: Vector3(24.0, 0.0, -18.0),
		LumenfallTypes.InteractionId.ORIN_VOICE: Vector3(0.0, 0.0, 28.0),
	}
	for altar_id: LumenfallTypes.InteractionId in altar_data:
...
@private
func _build_altar(altar_id: LumenfallTypes.InteractionId, at: Vector3) -> void:
	var root := Node3D.new()
	root.name = String(LumenfallTypes.InteractionId.find_key(altar_id)).to_pascal_case()
	root.position = at
	add_child(root)
...
func _build_crossing_gate() -> void:
	...
	crossing_gate.configure(LumenfallTypes.InteractionId.THREE_VOICES_GATE, "Open the Crossing of Three Voices")
	crossing_gate.position = gate.position
	add_child(crossing_gate)


func awaken_altar(altar_id: LumenfallTypes.InteractionId) -> void:
