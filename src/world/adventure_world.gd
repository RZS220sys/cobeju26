...
func _ready() -> void:
	...
	add_child(village)
	_configure_village_schedules()
	village.apply_crossing_choice(profile)
	...
	_hud.set_lens_owned(AdventureInventory.count(profile, LumenfallTypes.ItemId.LANTERN_LENS) > 0)
...
@private
func _configure_village_schedules() -> void:
	var nia: AdventureNpc = village.npcs.get(LumenfallTypes.NpcId.NIA) as AdventureNpc
	var bram: AdventureNpc = village.npcs.get(LumenfallTypes.NpcId.BRAM) as AdventureNpc
	var mara: AdventureNpc = village.npcs.get(LumenfallTypes.NpcId.MARA) as AdventureNpc
	var pip: AdventureNpc = village.npcs.get(LumenfallTypes.NpcId.PIP) as AdventureNpc
	if is_instance_valid(nia):
		nia.configure_schedule([Vector3(2.2, 0.0, -12.8), Vector3(-1.8, 0.0, -13.2), Vector3(1.0, 0.0, -10.8)] as Array[Vector3], 1.0)
	if is_instance_valid(bram):
		bram.configure_schedule([Vector3(8.0, 0.0, -2.0), Vector3(10.2, 0.0, -1.0), Vector3(8.8, 0.0, -4.0)] as Array[Vector3], 1.05)
	if is_instance_valid(mara):
		mara.configure_schedule([Vector3(-5.5, 0.0, 5.0), Vector3(4.0, 0.0, 6.0), Vector3(15.0, 0.0, 2.8), Vector3(4.0, 0.0, 2.5)] as Array[Vector3], 1.5)
	if is_instance_valid(pip):
		pip.configure_schedule([Vector3(2.0, 0.0, 4.0), Vector3(-4.0, 0.0, 3.5), Vector3(-3.0, 0.0, -3.0), Vector3(4.0, 0.0, -3.0)] as Array[Vector3], 2.0)
...
func _on_interaction_requested() -> void:
	if is_instance_valid(_focused_interactable):
		...
		elif profile.quest_stage >= LumenfallTypes.QuestStage.RESCUE_MISSING_VILLAGERS:
...
func _update_navigation() -> void:
	var target_node := _navigation_target()
	if not is_instance_valid(target_node):
		_hud.set_navigation("", "", 0.0)
		return
	var offset := target_node.global_position - player.global_position
	offset.y = 0.0
	var distance := offset.length()
	if distance < 0.01:
		_hud.set_navigation("◆", _navigation_target_name(), 0.0)
		return
	var forward := -camera_rig.get_player_camera().global_basis.z
	forward.y = 0.0
	forward = forward.normalized()
	var right := camera_rig.get_player_camera().global_basis.x
	right.y = 0.0
	...
	if absf(angle) > 2.45:
		...
		glyph = "←"
	_hud.set_navigation(glyph, _navigation_target_name(), distance)


@private
func _navigation_target() -> Node3D:
	match profile.quest_stage:
		LumenfallTypes.QuestStage.SUMMONED:
			return village.npcs.get(LumenfallTypes.NpcId.NIA) as Node3D
		LumenfallTypes.QuestStage.WAKE_WAYSTONE, LumenfallTypes.QuestStage.BRING_COMPASS_TO_WAYSTONE:
			return village.waystone_interactable
		LumenfallTypes.QuestStage.MEET_MARA, LumenfallTypes.QuestStage.SHOW_LENS_TO_MARA:
			return village.npcs.get(LumenfallTypes.NpcId.MARA) as Node3D
		LumenfallTypes.QuestStage.RETURN_SHARDS_TO_BRAM:
			return village.npcs.get(LumenfallTypes.NpcId.BRAM) as Node3D
		LumenfallTypes.QuestStage.CRAFT_LANTERN_LENS, LumenfallTypes.QuestStage.CRAFT_MEMORY_COMPASS:
		...
		LumenfallTypes.QuestStage.DEFEND_LANTERN_ROAD:
			return _quest._first_hound
		LumenfallTypes.QuestStage.LISTEN_TO_ASTER:
			return _quest._hound_memory
		LumenfallTypes.QuestStage.REPORT_ASTER, LumenfallTypes.QuestStage.REPORT_THREE_INSTRUMENTS, LumenfallTypes.QuestStage.RETURN_THREE_NAMES, LumenfallTypes.QuestStage.REPORT_CROSSING_ANSWER:
			return village.npcs.get(LumenfallTypes.NpcId.NIA) as Node3D
		LumenfallTypes.QuestStage.CHOOSE_REGION, LumenfallTypes.QuestStage.MARK_REGION_DISCOVERY:
			return village.field_map
		LumenfallTypes.QuestStage.DISCOVER_REGION_INSTRUMENT:
			return _selected_region_landmark()
		LumenfallTypes.QuestStage.ANSWER_CAUSEWAY_GATE:
			return echo_causeway.crossing_gate if is_instance_valid(echo_causeway) else null
		LumenfallTypes.QuestStage.EXAMINE_CROSSING_AFTERMATH:
			return village.crossing_aftermath
		LumenfallTypes.QuestStage.FOLLOW_GLASS_ROAD:
			return region_landmarks.landmarks.get(LumenfallTypes.InteractionId.GLASS_ROAD_BEACON) as Node3D
	return _closest_active_story_node()


@private
func _selected_region_landmark() -> Node3D:
	match profile.active_quest_id:
		LumenfallTypes.QuestId.GLASSWOOD_CALL:
			return region_landmarks.landmarks.get(LumenfallTypes.InteractionId.GLASSWOOD_SHRINE) as Node3D
		LumenfallTypes.QuestId.AMBERFEN_ORBIT:
			return region_landmarks.landmarks.get(LumenfallTypes.InteractionId.AMBERFEN_ORRERY) as Node3D
		LumenfallTypes.QuestId.BELLSCAR_WAKE:
			return region_landmarks.landmarks.get(LumenfallTypes.InteractionId.BELLSCAR_BELL) as Node3D
	return null


@private
func _closest_active_story_node() -> Node3D:
	var closest: Node3D
	var closest_distance := INF
	var candidates: Array[Node3D] = []
	if profile.quest_stage == LumenfallTypes.QuestStage.GATHER_RIFTGLASS:
		for shard: AdventureRiftglassShard in _quest._spawned_shards:
			if is_instance_valid(shard) and shard.is_in_group(&"adventure_interactables"):
				candidates.append(shard)
	elif profile.quest_stage == LumenfallTypes.QuestStage.RESCUE_MISSING_VILLAGERS:
		for npc: AdventureNpc in _names_quest.missing_people.values():
			if is_instance_valid(npc) and npc.is_in_group(&"adventure_interactables"):
				candidates.append(npc)
	elif profile.quest_stage == LumenfallTypes.QuestStage.AWAKEN_CAUSEWAY_VOICES and is_instance_valid(echo_causeway):
		for altar: AdventureWorldInteractable in echo_causeway.altars.values():
			if is_instance_valid(altar) and altar.is_in_group(&"adventure_interactables"):
...
@private
func _navigation_target_name() -> String:
	match profile.quest_stage:
		LumenfallTypes.QuestStage.SUMMONED: return "Nia"
		LumenfallTypes.QuestStage.WAKE_WAYSTONE, LumenfallTypes.QuestStage.BRING_COMPASS_TO_WAYSTONE: return "Waystone"
		LumenfallTypes.QuestStage.MEET_MARA, LumenfallTypes.QuestStage.SHOW_LENS_TO_MARA: return "Mara"
		LumenfallTypes.QuestStage.GATHER_RIFTGLASS: return "Singing shard"
		LumenfallTypes.QuestStage.RETURN_SHARDS_TO_BRAM: return "Bram"
		LumenfallTypes.QuestStage.CRAFT_LANTERN_LENS, LumenfallTypes.QuestStage.CRAFT_MEMORY_COMPASS: return "Lantern bench"
		LumenfallTypes.QuestStage.REACH_EASTERN_CROSSING: return "Eastern lanterns"
		LumenfallTypes.QuestStage.DEFEND_LANTERN_ROAD: return "Rift hound"
		LumenfallTypes.QuestStage.LISTEN_TO_ASTER: return "Hound memory"
		LumenfallTypes.QuestStage.REPORT_ASTER, LumenfallTypes.QuestStage.REPORT_THREE_INSTRUMENTS, LumenfallTypes.QuestStage.RETURN_THREE_NAMES, LumenfallTypes.QuestStage.REPORT_CROSSING_ANSWER: return "Nia"
		LumenfallTypes.QuestStage.CHOOSE_REGION, LumenfallTypes.QuestStage.MARK_REGION_DISCOVERY: return "Field map"
		LumenfallTypes.QuestStage.DISCOVER_REGION_INSTRUMENT:
			if profile.active_quest_id == LumenfallTypes.QuestId.GLASSWOOD_CALL: return "Glasswood Shrine"
			if profile.active_quest_id == LumenfallTypes.QuestId.AMBERFEN_ORBIT: return "Amberfen Orrery"
			return "Bellscar Bell"
		LumenfallTypes.QuestStage.RESCUE_MISSING_VILLAGERS: return "Missing villager"
		LumenfallTypes.QuestStage.AWAKEN_CAUSEWAY_VOICES: return "Sleeping voice"
		LumenfallTypes.QuestStage.ANSWER_CAUSEWAY_GATE: return "Central gate"
		LumenfallTypes.QuestStage.EXAMINE_CROSSING_AFTERMATH: return "Changed Waystone"
		LumenfallTypes.QuestStage.FOLLOW_GLASS_ROAD: return "Western light"
	return ""
