class_name EchoCauseway
extends Node3D

var altars: Dictionary[InteractionCatalog.Id, WorldInteractable] = {}
var crossing_gate: WorldInteractable
...
func _build_causeway() -> void:
	...
	var altar_data: Dictionary[InteractionCatalog.Id, Vector3] = {
		InteractionCatalog.Id.IVEN_VOICE: Vector3(-24.0, 0.0, -18.0),
		InteractionCatalog.Id.SOLA_VOICE: Vector3(24.0, 0.0, -18.0),
		InteractionCatalog.Id.ORIN_VOICE: Vector3(0.0, 0.0, 28.0),
	}
	for altar_id: InteractionCatalog.Id in altar_data:
...
func _add_floor(at: Vector3, size: Vector3, yaw: float = 0.0) -> void:
	...
	mesh.size = size
	visual.mesh = mesh
	visual.material_override = ModelLibrary.material(Color("272e3a"), 0.72, Color("213b55"), 0.22)
	body.add_child(visual)
	add_child(body)


@private
func _build_altar(altar_id: InteractionCatalog.Id, at: Vector3) -> void:
	var root := Node3D.new()
	root.name = String(InteractionCatalog.Id.find_key(altar_id)).to_pascal_case()
	root.position = at
	add_child(root)
	for index: int in range(3):
		...
		pillar.material_override = ModelLibrary.material(Color("5a4c86"), 0.35, Color("7661bd"), 0.8)
...
func _build_crossing_gate() -> void:
	...
	for side: int in [-1, 1]:
		...
		pillar.material_override = ModelLibrary.material(Color("343849"), 0.78)
	...
	lintel.material_override = ModelLibrary.material(Color("343849"), 0.78)
	gate.add_child(lintel)
	crossing_gate = WorldInteractable.new()
	crossing_gate.name = "ThreeVoicesGateInteractable"
	crossing_gate.configure(InteractionCatalog.Id.THREE_VOICES_GATE, "Open the Crossing of Three Voices")
	crossing_gate.position = gate.position
	add_child(crossing_gate)


func awaken_altar(altar_id: InteractionCatalog.Id) -> void:
	var interactable: WorldInteractable = altars.get(altar_id) as WorldInteractable
