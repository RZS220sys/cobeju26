...
func _build_forest_cover() -> void:
	...
	for index: int in range(tree_count):
		...
		var asset_id := LumenfallTypes.AssetId.PINE if random.randf() < pine_chance else LumenfallTypes.AssetId.OAK
		var tree := AdventureAssetLibrary.instantiate_model(asset_id)
		if not is_instance_valid(tree):
			continue
		tree.name = "Wild%s_%d" % [LumenfallTypes.asset_file_name(asset_id).capitalize(), index]
