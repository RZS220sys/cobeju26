...
func _ready() -> void:
	...
	collision_mask = PhysicsLayers.Id.NONE
...
func _build_forest_cover() -> void:
	...
	for index: int in range(tree_count):
		...
		var asset_id := ModelCatalog.Id.PINE if random.randf() < pine_chance else ModelCatalog.Id.OAK
		var tree := ModelLibrary.instantiate_model(asset_id)
		if not is_instance_valid(tree):
			continue
		tree.name = "Wild%s_%d" % [ModelCatalog.file_name(asset_id).capitalize(), index]
		...
		trunk.collision_layer = PhysicsLayers.Id.WORLD
